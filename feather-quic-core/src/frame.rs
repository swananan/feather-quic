...
impl QuicFrame {
        ...
        match self {
            ...
            QuicFrame::Stream(stream_frame) => {
                ...
                if stream_frame.is_fin {
                    type_bits = type_bits | 0x01;
                }

                if stream_frame.length > 0 {
                    type_bits = type_bits | 0x02;
                }

                if stream_frame.offset > 0 {
                    type_bits = type_bits | 0x04;
                }
                ...
                if stream_frame.length > 0 {
                    ...
                    encode_variable_length(cursor, writen_len)?;
                    cursor.write_all(
                    ...
                    if writen_len != stream_frame.length {
                        ...
                        let new_frame = QuicFrame::create_stream_frame(
                            Some(new_data),
                            stream_frame.offset + writen_len as u64,
    ...
    pub(crate) fn create_streams_blocked_frame(maximum_streams: u64) -> QuicFrame {
        ...
        while (cursor.position() as usize) < pbuf.len() {
            ...
            match frame_type {
                ...
                QuicFrameType::Stream => {
                    Self::handle_stream_frame(&mut cursor, qconn, frame_type_val as u8)?
                }
                ...
                QuicFrameType::MaxStreams => {
                    Self::handle_max_streams_frame(&mut cursor, qconn, frame_type_val as u8)?
                }
                ...
                QuicFrameType::StreamsBlocked => {
                    Self::handle_streams_blocked_frame(&mut cursor, qconn)?
                }
    ...
    fn handle_max_streams_frame(
        ...
        let is_bidirectional = if type_bits == 0x12 {
            true
        } else if type_bits == 0x13 {
            false
        } else {
            unreachable!();
        };
    ...
    fn handle_streams_blocked_frame(
        ...
        trace!(
            "Got streams blocked frame, maximum_streams {}",
            maximum_streams
        );

        qconn.handle_streams_blocked_frame(maximum_streams)
    }
