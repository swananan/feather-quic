...
impl QuicConnectionFlowControl {
    ...
    pub(crate) fn check_if_update_max_recv_data(&mut self, do_it_anyway: bool) -> bool {
        if do_it_anyway {
            if self.get_new_max_recv_size() > self.connection_recv_max_data {
                return true;
            } else {
                return false;
            }
    ...
    pub(crate) fn handle_max_streams_frame(
        ...
        if is_bidirectional {
            ...
            if max_streams > self.remote_max_uni_streams {
                info!(
...
impl QuicStreamFlowControl {
    ...
    pub(crate) fn get_sent_acked(&self) -> u64 {
        self.sent_acked
    }
    ...
    pub(crate) fn check_if_update_max_recv_size(&mut self, do_it_anyway: bool) -> bool {
        if do_it_anyway {
            if self.get_new_max_recv_size() > self.max_recv_size {
                return true;
            } else {
                return false;
            }
        ...
        match self.get_recv_available_bytes() {
            Ok(bytes) => {
                if bytes >= self.max_recv_size / 2 {
                    ...
                    self.max_recv_size = self.recv_offset + self.max_recv_size;
                    true
                } else {
                    false
                }
...
mod tests {
    ...
    fn test_connection_flow_control_recv_operations() {
        ...
        assert_eq!(flow_control.check_if_update_max_recv_data(false), false);
        assert_eq!(flow_control.check_if_update_max_recv_data(true), true);
    }
