...
pub enum QuicStreamError {
    ...
    InvalidSendingStreamState(QuicSendingStreamState, QuicSendingStreamState),
}
...
impl QuicStream {
    pub(crate) fn new(stream_id: QuicStreamHandle, max_send_size: u64, max_recv_size: u64) -> Self {
        ...
        info!("Created new stream {}", stream_id,);
    ...
    pub(crate) fn send(&mut self, snd_buf: &[u8]) -> Result<usize, QuicStreamError> {
        ...
        trace!(
            "Quic stream {} is trying to send {} bytes, actually sent {} bytes, \
            totally sent {} bytes, max send size {} bytes",
            self.stream_id,
            snd_buf.len(),
            sent_bytes,
    ...
    pub(crate) fn finish(&mut self) -> Result<(), QuicStreamError> {
            ...
            && !matches!(self.send_state, QuicSendingStreamState::Send)
        {
            ...
            return Err(QuicStreamError::InvalidSendingStreamState(
                QuicSendingStreamState::DataSent,
                self.send_state,
            ));
        }
    ...
    pub(crate) fn stop_sending(
        ...
        info!(
            ...
            self.stream_id, application_error_code
        );
    ...
    pub(crate) fn consume_send_queue(
        ...
        match self.send_queue.take() {
            Some(mut send_queue) => {
                ...
                let fin = self.fin;
                let send_len = if *connection_limit < send_queue.len() as u64 {
                    let tmp = *connection_limit;
                    let new_queue = send_queue.split_off(tmp as usize);
                    *connection_limit = 0;
                    trace!(
                        "Split send queue: original size={}, split at={}, remaining={}",
                    ...
                    *connection_limit = *connection_limit - send_queue.len() as u64;
                ...
                info!(
                    "Consumed send queue: stream={}, offset={}, length={}, fin={}, remaining_limit={}",
                    self.stream_id,
                    offset,
                    send_len,
                    fin,
    ...
    pub(crate) fn ack_stream(&mut self, frame: &QuicStreamFrame) -> Result<()> {
        ...
        if frame.is_fin {
            ...
            info!("Stream {} fin flag has been acked", self.stream_id);
        }
            ...
            && self.flow_control.get_sent_in_flight()? == 0
        {
            ...
            info!("Stream {} sender job has been done", self.stream_id);
    ...
    pub(crate) fn handle_stop_sending_frame(
        ...
        info!(
            "Stream {} sending was reset by peer with error code {} \
            , final size {}",
            self.stream_id,
            application_error_code,
            self.flow_control.get_sent_offset(),
    ...
    pub(crate) fn handle_stream_frame(
        ...
        if fin_bit {
            info!("Stream {} received STREAM + FIN", self.stream_id);
        ...
        self.recv_bufs
            .insert(&cursor.get_ref()[..length as usize], offset);
    ...
    fn set_recv_state(&mut self, state: QuicReceivingStreamState) {
        ...
        info!(
            ...
            self.stream_id, self.recv_state, state
        );
    ...
    fn set_send_state(&mut self, state: QuicSendingStreamState) {
        ...
        info!(
            ...
            self.stream_id, self.send_state, state
        );
