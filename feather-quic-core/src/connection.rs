...
use crate::utils::{decode_variable_length, format_instant};
...
impl QuicConnection {
    ...
    pub fn open_stream(
                ...
                .unwrap_or(true)
        {
            return Err(QuicConnectionError::StreamLimitations(
                ...
                self.flow_control.get_max_streams_bidi_local(),
            ));
        }
    ...
    pub fn stream_send(
        ...
        trace!(
            "Attempting to send {} bytes to stream {:?}",
            snd_buf.len(),
            stream_handle
        );

        let sent_bytes = stream.send(snd_buf)?;
    ...
    pub(crate) fn handle_streams_blocked_frame(&mut self, max_streams: u64) -> Result<()> {
        info!(
            "Handling STREAMS_BLOCKED frame: max_streams={}",
            max_streams
        );
        // Shall we send max streams frame?
        Ok(())
    }
    ...
    pub(crate) fn handle_stream_frame(
        ...
        let off_bit = type_bits & 0x04;
        let len_bit = type_bits & 0x02;
        let fin_bit = if type_bits & 0x01 > 0 { true } else { false };
        ...
        if self.flow_control.get_recv_available_bytes()? <= length {
            return Err(anyhow!("Receive connection flow control limit exceeded"));
        } else {
    ...
    fn recv_new_stream(
        ...
        info!(
            "Opening new stream from server side {:?} (directional: {}, max_send_size: {} \
            max_recv_size: {})",
            stream_id,
    ...
    fn consume_stream_send_queue(&mut self) -> Result<()> {
        // TODO: Manage streams' priorities
        let mut available_bytes = self.flow_control.get_sent_available_bytes()?;
        self.streams.iter_mut().try_for_each(|(_, stream)| {
            // Check Connection-level flow control
            if available_bytes == 0 {
                return ControlFlow::Break(());
            }
            if let Some((data, offset, stream_id, is_fin)) =
                stream.consume_send_queue(&mut available_bytes)
            {
                let send_ctx = &mut self.app_send;
                self.flow_control
                    .increment_sent_offset(data.as_ref().map(|v| v.len() as u64).unwrap_or(0));
