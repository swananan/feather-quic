...
impl FeatherQuicEchoContext {
    ...
    fn read_and_verify_response(
        ...
        match qconn.stream_recv(stream_handle, recv_len) {
            Ok(recv_bytes) => {
                ...
                let response_str = String::from_utf8_lossy(&recv_bytes);
                if response_str.contains('\n') {
                    // Verify the response
                    self.verify_response(stream_handle, &recv_bytes)?;

                    // Read and send next line if available
                    if let Some(line) = self.read_next_line(stream_handle)? {
                        if let Some(context) = self.stream_contexts.get_mut(&stream_handle) {
    ...
    fn verify_response(&mut self, stream_handle: QuicStreamHandle, response: &[u8]) -> Result<()> {
        if let Some(context) = self.stream_contexts.get(&stream_handle) {
            let response_str = String::from_utf8_lossy(response);
            if response_str != context.current_line {
                return Err(anyhow::anyhow!(
                    "Echo verification failed for stream {:?}. Expected: '{}', Got: '{}'",
                    stream_handle,
